...
# This is a naive baseline implementation. 
...
# A hard-coded rule that generate wh-questions
# Not finished
def sentences_to_whquestions(sentences):
    questions = []
...
def sentences_to_yesnoquestions(sentences):
    ...
    for sentence in sentences:
        ...
        prefix = pos_tags[(begin + 1) : i]
        prefix = [tag[0] for tag in prefix]
        suffix = pos_tags[(i + 1) : end]
        suffix = [tag[0] for tag in suffix]
        if filter(lambda noun: noun.lower() in keywords, prefix):
            question = reduce(lambda x, y: x + ' ' + y if y != "'s" and y != "'" else x + y, [pos_tags[i][0]] + prefix + suffix)
...
def get_questions_from_sentences(sentences, \
    ...
    questions = []
    for f in sentence_to_questions_functions:
        questions += f(sentences)
    return questions
...
def main():
    ...
    questions = get_questions_from_sentences(sentences_in_paragraphs, [sentences_to_whquestions, sentences_to_yesnoquestions])
    if verbose:
        print(questions)

    # Shuffle and print: don't use this
    questions = filter(lambda q: len(q.split(' ')) < 30, questions)
